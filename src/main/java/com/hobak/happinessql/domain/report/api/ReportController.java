...
public class ReportController {
    ...
    @Operation(summary = "[주간] 행복 종합 리포트", description = "이번 주 언제, 어디에서, 무엇을 할 때 행복했는지에 대한 종합적인 리포트를 제공합니다.")
    @GetMapping("/week/summary")
    public DataResponseDto<ReportSummaryResponseDto> getWeeklySummary(@AuthenticationPrincipal UserDetails userDetails) {
        User user = userFindService.findByUserDetails(userDetails);
        ReportSummaryResponseDto responseDto = reportSummaryService.getWeeklySummary(user);
        return DataResponseDto.of(responseDto, "행복 종합 리포트(주간)를 성공적으로 조회했습니다.");
    }
